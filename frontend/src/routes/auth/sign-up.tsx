...
//     if (!ts) return; // no session, do nothing

//     const timestamp = Number(ts);
//     const expired = Date.now() - timestamp > 5 * 60 * 1000; // 5 minutes in ms

//     if (expired) {
//       // Clear all signup session related storage keys
//       localStorage.removeItem("otp-timestamp");
//       localStorage.removeItem("otp-step");
//       localStorage.removeItem("mobile");
//       // (Add here any other keys you use in signup)
...
//       // Optional: Reset your local UI step state so user goes back to initial step
//       setStep("mobile");
//     }
...
import { motion } from "framer-motion";
import { setupRecaptcha, sendOtp } from "../../firebase/otpUtils";
//im//port { Loader as Spinner } from "lucide-react";
...
//  const [highlightMemberId, setHighlightMemberId] = useState<number | null>(null);
  ...
  const { seconds, active, start } = useOtpTimer(60);
  ...
  useEffect(() => {
    setupRecaptcha("recaptcha-container"); // ✅ only run once on mount
  }, []);
  ...
  // Automatically clear localStorage keys after 5 minutes
  useEffect(() => {
    const interval = setInterval(() => {
      const ts = localStorage.getItem("otp-timestamp");
      if (!ts) return;
      ...
      const timestamp = Number(ts);
      const expired = Date.now() - timestamp > 5 * 60 * 1000; // 5 minutes
  ...
  return (
    <div className="h-[calc(100vh-64px)] w-full flex bg-gradient-to-br from-black via-gray-900 to-green-900 relative overflow-hidden">
      ...
      >
        ...
        <motion.p className="mt-6 text-lg text-gray-300 leading-relaxed max-w-md">
          Learning opens the door to limitless opportunities. Let’s build something
          amazing together!
        </motion.p>
      </motion.div>
